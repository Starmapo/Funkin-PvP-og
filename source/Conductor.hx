...
class Conductor
{
	...
	{
		return normalize(crochet);
	}

	public static function get_normalizedStepCrochet():Float
	{
		return normalize(stepCrochet);
	}

	public static inline function normalize(a:Float) {
		return a * (timeSignature[1] / 4);
	}
	...
	public static function beatToSeconds(song:SwagSong, beat:Float):Float
	{
		...
		return lastChange.songTime
			+
			((step - lastChange.stepTime) / (lastChange.bpm / 60) / lastChange.timeSignature[1]) * 1000; // TODO: make less shit and take BPM into account PROPERLY
	}
