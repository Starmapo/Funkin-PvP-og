...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	function addSongUI():Void
	{
		...
		var difficultyDropDown = new FlxUIDropDownMenu(stageDropDown.x, gfVersionDropDown.y, FlxUIDropDownMenu.makeStrIdLabelArray(CoolUtil.difficulties, true), function(difficulty:String)
		{
			if (PlayState.storyDifficulty != Std.parseInt(difficulty)) {
				...
				try {
					PlayState.SONG = Song.loadFromJson(CoolUtil.formatSong(_song.song), _song.song);
					_song = PlayState.SONG;
					MusicBeatState.resetState();
				} catch (e) {
					trace('File ${CoolUtil.formatSong(_song.song)} was not found.');
				}
			}
		});
	...
	function updateGrid():Void
	{
		...
		for (i in _song.notes[curSec].sectionNotes)
		{
			...
			if (i[3] != null && note.noteType != null && note.noteType.length > 0) {
				...
				var daText:AttachedFlxText = new AttachedFlxText(100, theType, 24);
		...
		for (i in _song.events)
		{
			if (endThing > i[0] && i[0] >= startThing)
			{
				...
				var daText:AttachedFlxText = new AttachedFlxText(400, text, 12);
	...
	function loadJson(song:String):Void
	{
		...
		PlayState.SONG = Song.loadFromJson(CoolUtil.formatSong(song), song);
		_song = PlayState.SONG;
		trace('loading: ' + CoolUtil.formatSong(song));
		MusicBeatState.resetState();
	}
	...
	private function saveLevel()
	{
		...
		var savedSong:SwagSong = {
			...
			type: 'psychextra',
			sliderVelocities: _song.sliderVelocities
		};
		var json = {
			"song": savedSong
		};
		...
		if ((data != null) && (data.length > 0))
		{
			...
			_file.save(data.trim(), '${CoolUtil.formatSong(_song.song)}.json');
		}
	}
