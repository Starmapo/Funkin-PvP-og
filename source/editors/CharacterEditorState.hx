		...
		var removeButton:FlxButton = new FlxButton(180, animationIndicesInputText.y + 30, "Remove", function() {
			for (anim in char.animationsArray) {
				if (animationInputText.text == anim.anim) {
					...
					if (char.animation.curAnim == null || !char.animation.exists(anim.anim) || (char.animation.curAnim != null && anim.anim == char.animation.name)) resetAnim = true;
		...
		if (!charDropDown.dropPanel.visible) {
			...
			if (char.animationsArray.length > 0) {
				...
				if (FlxG.keys.justPressed.T)
				{
					...
					char.setOffsets();
					if (ghostChar.animation.curAnim != null && char.animation.curAnim != null && char.animation.name == ghostChar.animation.name) {
						ghostChar.setOffsets();
					}
					genBoyOffsets();
				}
