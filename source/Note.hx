...
class Note extends FNFSprite
{
	...
	private function set_multSpeed(value:Float):Float
	{
		...
		multSpeed = value;
		initialPos = PlayState.getPosFromTime(strumTime, multSpeed);
		return value;
	}

	public function resizeByRatio(ratio:Float) // haha funny twitter shit
	{
		if (isSustainNote && animation.curAnim != null && !animation.name.endsWith('end'))
		{
			scale.y *= ratio;
			updateHitbox();
			defaultScale.y *= ratio;
		}
	}
	...
	private function set_noteType(value:String):String
	{
		if (value != null && noteData > -1)
		{
			...
			if (colorSwap != null && noteData < ClientPrefs.arrowHSV[keyAmount - 1].length)
			{
		...
		if (isSustainNote && prevNote != null)
		{
			...
			if (prevNote.isSustainNote)
			{
				...
				if (CoolUtil.inPlayState())
					prevNote.scale.y *= PlayState.getFNFSpeed(prevNote.strumTime, multSpeed);
				...
				prevNote.defaultScale.y = prevNote.scale.y;
			}
			if (skinModifier.endsWith('pixel'))
			{
