...
class ControlsSubState extends MusicBeatSubState {
	...
	var keyTexts:Array<Array<String>> = [['4K']];
				...
				if (optionShit[curSelected][0] == defaultKey) {
					...
					for (i in keyTexts) {
						if (optionShit[curSelected] == i) {
							openSubState(new NoteBindingSubState(4));
							match = true;
						}
					}
...
class NoteBindingSubState extends MusicBeatSubState {
			...
			if (!testingBinds) {
				...
				if (controlArray.contains(true)) {
					for (i in 0...controlArray.length) {
						if (controlArray[i]) {
							strumGroup.getReceptor(i).playAnim('pressed');
						}
					}
				}
				...
				if (controlReleaseArray.contains(true)) {
					for (i in 0...controlReleaseArray.length) {
						if (controlReleaseArray[i]) {
							strumGroup.getReceptor(i).playAnim('static');
						}
					}
				}
	...
	function changeSelection(change:Int = 0) {
		...
		for (i in 0...strumGroup.receptors.length) {
			if (i == curSelected) {
				strumGroup.getReceptor(i).alpha = 1;
			} else {
				strumGroup.getReceptor(i).alpha = 0.6;
			}
		}
