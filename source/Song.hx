...
class Song
{
	private static function onLoadJson(songJson:Dynamic) // Convert old charts to newest format
	{
		...
		for (secNum in 0...songJson.notes.length)
		{
			...
			if (sec.gfSection == null)
				sec.gfSection = false;
			if (sec.bpm == null)
				sec.bpm = songJson.bpm;
			if (sec.changeBPM == null)
				sec.changeBPM = false;
			...
			if (sec.changeSignature == null)
				sec.changeSignature = false;
			if (sec.altAnim == null)
				sec.altAnim = false;
			if (sec.changeKeys == null)
				sec.changeKeys = false;
			if (sec.boyfriendKeyAmount == null)
				sec.boyfriendKeyAmount = songJson.boyfriendKeyAmount;
			if (sec.dadKeyAmount == null)
				sec.dadKeyAmount = songJson.dadKeyAmount;

			if (sec.changeKeys)
				curKeys = sec.boyfriendKeyAmount + sec.dadKeyAmount;
	...
	{
		var daJson = Json.parse(rawJson);
		var swagShit:SwagSong = cast daJson.song; // actual song
		var tempSong:Dynamic = cast daJson.song; // copy to check for other variables
		...
		if (daJson.sliderVelocities != null) {
			var shit:Array<VelocityChange> = cast daJson.sliderVelocities;
				...
				if (swagNote.exists)
				{
					...
					if (floorSus > 0)
					{
						for (susNote in 0...floorSus + 1)
						{
							...
							var sustainNote:Note = new Note(songNotes[0] + (curStepCrochet * susNote) + (curStepCrochet / songSpeed), daNoteData, oldNote,
								true, keys);
		...
		if (Paths.existsPath('data/$curSong/events.json', TEXT))
		{
			...
			for (event in eventsData) // Event Notes
			{
				for (i in 0...event[1].length)
				{
					...
					if (earlyTriggerFunction != null)
						subEvent.strumTime -= earlyTriggerFunction(subEvent);
					eventNotes.push(subEvent);
					if (pushedCallback != null)
						pushedCallback(subEvent);
				}
			}
		}
	...
	public static function getMetaFile(name:String):MetaFile
	{
		...
		if (json.iconHiddenUntilPlayed == null)
			json.iconHiddenUntilPlayed = true;
		return json;
	}
