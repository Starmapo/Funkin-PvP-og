...
class Stage extends FlxBasic {
    ...
    public var smonk1:FlxSprite;
    public var smonk2:FlxSprite;
    public var smonk3:FlxSprite;
	public var smonk4:FlxSprite;
	public var smokeOverlay:FlxSprite;
        ...
        if (ClientPrefs.gameQuality != 'Crappy') {
            switch (curStage)
            {
                ...
                case 'tank': //Week 7 - Ugh, Guns, Stress
                    ...
                    foreground.add(new BGSprite('tank/tank0', -500, 650, 1.7, 1.5, ['fg']));
					if (ClientPrefs.gameQuality == 'Normal')
						foreground.add(new BGSprite('tank/tank1', -300, 750, 2, 0.2, ['fg']));
                    foreground.add(new BGSprite('tank/tank2', 450, 940, 1.5, 1.5, ['foreground']));
					if (ClientPrefs.gameQuality == 'Normal')
						foreground.add(new BGSprite('tank/tank4', 1300, 900, 1.5, 1.5, ['fg']));
					foreground.add(new BGSprite('tank/tank5', 1620, 700, 1.5, 1.5, ['fg']));
					if (ClientPrefs.gameQuality == 'Normal')
						foreground.add(new BGSprite('tank/tank3', 1300, 1200, 3.5, 2.5, ['fg']));
                ...
                case '!VOID!':
                    voidBG = new BGSprite('void', -700, -350, 0.5, 0.5);
					background.add(voidBG);

					var voidStatic = new BGSprite('voidStatic', -700, -350, 0.5, 0.5, ['Symbol 1'], true);
					voidStatic.setGraphicSize(Std.int(voidBG.width), Std.int(voidBG.height));
                    voidStatic.updateHitbox();
                    voidStatic.alpha = 0.2;
					background.add(voidStatic);

					glitchEffect = new Shaders.GlitchEffect();
					glitchEffect.waveAmplitude = 0.1;
					glitchEffect.waveFrequency = 5;
					glitchEffect.waveSpeed = 2;

					voidBG.shader = glitchEffect.shader;

                case 'hillzoneTails':
                    var bg = new BGSprite('tgt/chapter1/sky', -600, -200, 0.4, 0.4);
                    background.add(bg);

					var stageFront = new BGSprite('tgt/chapter1/grass', -650, 600, 0.9, 0.9);
					stageFront.setGraphicSize(Std.int(stageFront.width * 1.1));
					stageFront.updateHitbox();
					background.add(stageFront);

					var stageCurtains = new BGSprite('tgt/chapter1/foreground', -450, -150, 1.3, 1.3);
					stageCurtains.setGraphicSize(Std.int(stageCurtains.width * 0.87));
					stageFront.updateHitbox();
					foreground.add(stageCurtains);

				case 'hillzoneSonic':
					var bg = new BGSprite('tgt/chapter2/sky', -600, -200, 0.4, 0.4);
					background.add(bg);

					var stageFront = new BGSprite('tgt/chapter2/grass', -650, 600, 0.9, 0.9);
					stageFront.setGraphicSize(Std.int(stageFront.width * 1.1));
					stageFront.updateHitbox();
					background.add(stageFront);

					var stageCurtains = new BGSprite('tgt/chapter2/foreground', -450, -150, 1.3, 1.3);
					stageCurtains.setGraphicSize(Std.int(stageCurtains.width * 0.87));
					stageFront.updateHitbox();
					foreground.add(stageCurtains);

                case 'hillzoneShadow':
					var bg = new BGSprite('tgt/chapter3/shadowbg', -835, -550, 1.05, 1.05);
					background.add(bg);

					var thisthing = new BGSprite('tgt/chapter3/shadowbg3', -880, -730, 1.025, 1.025);
					background.add(thisthing);

					var thisotherthing = new BGSprite('tgt/chapter3/shadowbg2', -815, -375, 1.025, 1.025);
					background.add(thisotherthing);

					var grass = new BGSprite('tgt/chapter3/shadowbg4', -815, 450);
					background.add(grass);

                    if (ClientPrefs.gameQuality == 'Normal') {
						tails = new FlxSprite(200, 60);
						tails.frames = Paths.getSparrowAtlas('tgt/chapter3/tails');
						...
						background.add(tails);

						knuckles = new FlxSprite(770, 80);
						knuckles.frames = Paths.getSparrowAtlas('tgt/chapter3/knuckles');
						...
						knuckles.antialiasing = ClientPrefs.globalAntialiasing;
						overGF.add(knuckles);
                    }

                case 'highzoneShadow':
					var bg = new BGSprite('tgt/chapter3/stageback_HS', -350, -200, 0.4, 0.4);
					background.add(bg);

					var stageFront = new BGSprite('tgt/chapter3/stagefront_HS', -725, 600);
					stageFront.scale.set(1.1, 1.1);
					stageFront.updateHitbox();
					background.add(stageFront);

                    smonk1 = new FlxSprite(-1300, 625);
                    smonk1.frames = Paths.getSparrowAtlas('tgt/smoke');
                    smonk1.animation.addByPrefix('smoke', 'Symbol 7', 12);
                    smonk1.animation.play('smoke');
                    smonk1.scale.set(0.8, 0.8);
                    smonk1.updateHitbox();
                    smonk1.alpha = 0.00001;
					smonk1.antialiasing = ClientPrefs.globalAntialiasing;
                    foreground.add(smonk1);

					smonk2 = new FlxSprite(912.5, 625);
					smonk2.frames = Paths.getSparrowAtlas('tgt/smoke');
					smonk2.animation.addByPrefix('smoke', 'Symbol 7', 12);
					smonk2.animation.play('smoke');
					smonk2.scale.set(0.8, 0.8);
					smonk2.updateHitbox();
					smonk2.alpha = 0.00001;
                    smonk2.flipX = true;
					smonk2.antialiasing = ClientPrefs.globalAntialiasing;
					foreground.add(smonk2);

					smonk3 = new FlxSprite(-1400, 450);
					smonk3.frames = Paths.getSparrowAtlas('tgt/smoke');
					smonk3.animation.addByPrefix('smoke', 'Symbol 7', 13);
					smonk3.animation.play('smoke');
					smonk3.scale.set(0.5, 0.5);
					smonk3.updateHitbox();
					smonk3.alpha = 0.00001;
                    smonk3.scrollFactor.set(0.95, 0.95);
					smonk3.antialiasing = ClientPrefs.globalAntialiasing;
					background.add(smonk3);

					smonk4 = new FlxSprite(812.5, 450);
					smonk4.frames = Paths.getSparrowAtlas('tgt/smoke');
					smonk4.animation.addByPrefix('smoke', 'Symbol 7', 12);
					smonk4.animation.play('smoke');
					smonk4.scale.set(0.5, 0.5);
					smonk4.updateHitbox();
					smonk4.alpha = 0.00001;
					smonk4.flipX = true;
					smonk4.scrollFactor.set(0.95, 0.95);
                    smonk4.antialiasing = ClientPrefs.globalAntialiasing;
					background.add(smonk4);

                    smokeOverlay = new FlxSprite(0, 0, Paths.image('tgt/smokeoverlay'));
					smokeOverlay.scale.set(2, 2);
					smokeOverlay.updateHitbox();
                    smokeOverlay.screenCenter();
                    smokeOverlay.scrollFactor.set();
                    smokeOverlay.alpha = 0.00001;
                    foreground.add(smokeOverlay);

                case 'hillzoneDarkSonic':
					var sky = new BGSprite('tgt/chapter3/tfbbg3', -458, -247, 0.3, 0.3);
                    background.add(sky);

					var bg = new BGSprite('tgt/chapter3/tfbbg2', -480.5, 410, 0.7, 0.7);
					background.add(bg);

					var fg = new BGSprite('tgt/chapter3/tfbbg', -541, -96.5);
					background.add(fg);

				case 'hillzoneTailsSwag':
					var bg = new BGSprite('tgt/chapter1/skySwag', -600, -200, 0.4, 0.4);
					background.add(bg);

					var stageFront = new BGSprite('tgt/chapter1/grassSwag', -650, 600, 0.9, 0.9);
					stageFront.setGraphicSize(Std.int(stageFront.width * 1.1));
					stageFront.updateHitbox();
					background.add(stageFront);

                    if (PlayState.instance != null && PlayState.instance.curSong == 'tsuraran-fox') {
                        smonk1 = new FlxSprite(-1500, 665);
                        smonk1.frames = Paths.getSparrowAtlas('tgt/smoke');
                        smonk1.animation.addByPrefix('smoke', 'Symbol 7', 12);
                        smonk1.animation.play('smoke');
                        smonk1.scale.set(0.8, 0.8);
                        ...
                        smonk1.alpha = 0.00001;
                        smonk1.antialiasing = ClientPrefs.globalAntialiasing;
                        smonk1.scrollFactor.set(1.025, 1.025);
                        foreground.add(smonk1);

                        smonk2 = new FlxSprite(712.5, 665);
                        smonk2.frames = Paths.getSparrowAtlas('tgt/smoke');
                        smonk2.animation.addByPrefix('smoke', 'Symbol 7', 12);
                        smonk2.animation.play('smoke');
                        smonk2.scale.set(0.8, 0.8);
                        ...
                        smonk2.flipX = true;
                        smonk2.antialiasing = ClientPrefs.globalAntialiasing;
                        smonk2.scrollFactor.set(1.025, 1.025);
                        foreground.add(smonk2);
                    }

					var stageCurtains = new BGSprite('tgt/chapter1/foregroundSwag', -450, -150, 1.3, 1.3);
					stageCurtains.setGraphicSize(Std.int(stageCurtains.width * 0.87));
					stageFront.updateHitbox();
					foreground.add(stageCurtains);

                    if (PlayState.instance != null && PlayState.instance.curSong == 'tsuraran-fox') {
                        overlay = new FlxSprite(-460, -200, Paths.image('tgt/chapter1/spotlight'));
        ...
        if (ClientPrefs.gameQuality != 'Crappy') {
            switch (curStage) {
                case 'philly':
                    ...
                    phillyWindow.alpha -= (Conductor.normalizedCrochet / 1000) * elapsed * 1.5;
                
                case 'limo':
                    if (ClientPrefs.gameQuality == 'Normal') {
                        grpLimoParticles.forEach(function(spr:BGSprite) {
                            if (spr.animation.curAnim != null && spr.animation.finished) {
                                spr.kill();
                ...
                case 'schoolEvil':
                    if (ClientPrefs.gameQuality == 'Normal' && bgGhouls.animation.curAnim != null && bgGhouls.animation.finished) {
                        bgGhouls.visible = false;
                    }
                ...
                case '!VOID!':
					glitchEffect.shader.uTime.value[0] += elapsed;
            }
        }
    }
    ...
    public function onBeatHit() {
        if (ClientPrefs.gameQuality != 'Crappy') {
            ...
            switch (curStage) {
                ...
                case 'hillzoneShadow':
                    if (ClientPrefs.gameQuality == 'Normal' && curNumeratorBeat % 2 == 0) {
						if (knuckles.animation.curAnim != null
							&& knuckles.animation.name != 'shootKill'
							&& (knuckles.animation.name == 'idle' || knuckles.animation.finished))
                            knuckles.animation.play('idle', true);

                        if (tails.animation.curAnim != null
                            && tails.animation.name != 'shootKill'
                            && (tails.animation.name == 'idle' || tails.animation.finished))
                            tails.animation.play('idle', true);
                    }
            }
        }
